...
//
// 232M100 Module v4.0
//                v4.1  (AAK: Fixed LVP bug. 1/17/2010)
//
// Revision History
// ----------------------------------------------------------------------------
...
#FUSES CPUDIV2                  //Divide by 3 (96MHz PLL) 32MHz clock
...
//#fuses HSPLL, CPUDIV4, PLL5, NOWDT, NOPROTECT, BROWNOUT, PUT, NOLVP, DEBUG
...
int16 acquire_adc(int8 control)
{
    ...
    ad_sample = read_adc();
    ad_sample *= AD_Calibration[control];
    setup_adc(ADC_OFF);
...
//        value = value ^ 0xFF;
//
//    OUTPUT_B = value;             // bits 0-7 = PORTB 0-7
    return;
}
...
//    set_timer1(0xF199);                     // 1ms = 65536-(.001/(4/14745600)) = 0xF199
    set_timer1(0xF060);                     // 1ms = 65536-(.001/(4/(32000000/2))) = 0xF199
...
void check_async_stream(void)
{
    ...
    if (AsyncStreamFlags)
    {
        ...
        if (AsyncStreamUpdate)
        {
            ...
            {
                //
                // Analog sample.  The 232M100 only has 
...
{
    // Welcome message
    printf("\rIntegrity Instruments, Inc. Model 232M100 v4.1\r");
    return;
}
