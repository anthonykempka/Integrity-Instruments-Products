...
;
  __CONFIG   _CP_ALL & _WDT_ON & _HS_OSC & _PWRTE_ON & _BODEN_OFF
...
; ------------- BANK 0 ------------------------------------------------------
        ...
        andlw   0x08            ; SW Pos 1
        movwf   FAST_RESPONSE
        movwf   DIP_SWITCH      ; Get the RS232_FLAG bit
        andlw   0x01            ; SW Pos 2
        movwf   RS232_FLAG
...
Start_LED_On
        ...
        movlw   RamBank0+0X20   ; Set the RCV buffer pointer
        ...
        iorlw   0x00            ; Check if module address read as 0x00
        btfsc   STATUS, Z
        goto    Start_LED_On
        movfw   MODULE_ADDRESS
        sublw   0xFF            ; Check if module address read as 0xFF
        btfsc   STATUS, Z
        goto    Start_LED_On
...
Main_Loop
        clrwdt                  ; Kick the dog
        btfsc   PIR1, RCIF      ; Read incoming characters until a CR is rcvd
        goto    Main_Loop_Rcv_Char

; Check if there are any RCV errors
        btfsc   RCSTA, FERR
        goto    Main_Loop_Rcv_Error
        btfsc   RCSTA, OERR
        goto    Main_Loop_Rcv_Error
        goto    Main_Loop
Main_Loop_Rcv_Error
        bcf     RCSTA, CREN         ; Enable and Disable the port
        nop
        bsf     RCSTA, CREN
        movfw   RCREG               ; Read rcv register to clear errors
        incf    RCV_ERROR_CNT       ; Bump the error count
        goto    Main_Loop
...
Main_Loop_Parse
;
; Parse incoming message, check dest. address and SAVE source address
;
...
Main_Loop_Got_Address
        ...
        movwf   SRC_ADR
        bcf     STATUS, C       ; Make sure C is cleared
        rlf     SRC_ADR, F      ; Rotate to high order niblle location
        rlf     SRC_ADR, F
        rlf     SRC_ADR, F
        rlf     SRC_ADR, F
        movfw   RCV_BUFFER+3
...
Main_Loop_Check_Q
        movlw   'Q'                 ; QUERY ANALOG command
        xorwf   RCV_BUFFER+4, W
        btfss   STATUS, Z
        goto    Main_Loop_Check_N

        movfw   RCV_BUFFER+5        ; Get the ANALOG CONFIG nibble
        call    Hex_To_Nibble
        call    Acquire_Sample
...
Main_Loop_Check_N
        ...
        movwf   OUR_TMR0            ; save local copy so count does occur between
        andlw   0xF0                ; MSB read and LSB read
        movwf   SCRATCH             ; Rotate to LSB
        rrf     SCRATCH, F
        rrf     SCRATCH, F
        rrf     SCRATCH, F
...
Main_Loop_Check_L
        movlw   'L'                 ; CLEAR CAPTURE COUNT command
        xorwf   RCV_BUFFER+4, W
        btfss   STATUS, Z
...
Main_Loop_Check_T
        movlw   'T'                 ; DIGITAL DIRECTION command
        xorwf   RCV_BUFFER+4, W
        btfss   STATUS, Z
        ...
        call    Transmit_No_Delay   ; Transmit Addresses
...
Main_Loop_Check_U
        movlw   'U'                 ; CLEAR RCV ERROR COUNT command
        xorwf   RCV_BUFFER+4, W
        btfss   STATUS, Z
...
Main_Loop_Rcv_End
        ...
        movlw   RamBank0+0X20       ; Reset the RCV buffer pointer
        movwf   RCV_BUFFER_PTR
        movfw   RCREG               ; Read register to clear errors and any data
        movfw   RCREG               ; Read register to clear errors and any data
...
;
; ADC Configure nibble in W
;
; Data return in ADC_NIBBLE1/2/3
;
Acquire_Sample
        movwf   SCRATCH                 ; Save off CONFIG
        movlw   0x31                    ; SPI Master mode, CKP=1, Fosc/16
        movwf   SSPCON
        bcf     PORTA, ADC_CS           ; Select the ADC
        movfw   SCRATCH                 ; Restore CONFIG in W
...
Timer1_Isr_Cnt
        incf    RUNNING_CNT_L, F        ; Bump the LSB counter
        btfss   STATUS, Z
        goto    Timer1_Isr_End
        incf    RUNNING_CNT_H, F        ; Every 255 milliseconds gets here

Timer1_Isr_LED
        movfw   LED_ON_FLAG             ; Handle the LED blink
        xorlw   LED_ON
        btfss   STATUS, Z
        goto    Timer1_Led_Off
        incf    LED_CNT_L, F
...
; Sync_Isr
;
; Synchronous data reception ISR
;----------------------------------------------------------------------------
;
Sync_Rcv_Isr
