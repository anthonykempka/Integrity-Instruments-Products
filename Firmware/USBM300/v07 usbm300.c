...
int16 acquire_adc(int8 control, int8 unipolar)
{
    ...
    // Lock the CPU for exclusive access. Don't want USB to interrupt us here
    // since the interface is SPI and will result in a collision
...
{
    LedGreenFlag = 0;
    LedCount = 75;          // To make a faster Red LED blink, put a value here (0 ... 100)
                            // Full blink gets a 0
    led_on_red();
}
...
//-----------------------------------------------------------------------------
// update_eeflags - updates the EEPROM based flags from the EEPROM values
//
void update_eeflags(void)
{
    int8 i;
    //
    ...
    while (TRUE)
    {
        ...
        if (CommandBuffer[0] == 0) {
            ...
            if (StreamFlag) {
                ...
                // Collect the A/D samples
                if (AdcIndex == 0) {
                    for (AdcIndex=0; AdcIndex<24; AdcIndex++) {
                        AdcSamples[AdcIndex] = acquire_adc(8, 1);
                    }
                }
        ...
        switch (CommandBuffer[0]) {
            ...
            case 'W':
                ...
                //
                // Get new EEPROM Flag settings
                //
//                if ((i == EEPROM_EXPANDER_FLAG) || (i == EEPROM_AD_SAMPLE_CLK))
//                    update_eeflags();
