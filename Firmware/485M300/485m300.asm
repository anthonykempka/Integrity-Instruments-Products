...
; Description
; -----------
; Integrity Instruments 232M300 Module Firmware
;
; Version:  3.0
;
...
;   A. Kempka   v3.0        10/22/2002  Converted to 485M300 module
...
; BUGBUG: Use 16C65B in the final release
;
        list p=16c65B, f=inhx8m
...
EE_ASYNC_CONFIG_MSB EQU     0x04    ; ** Not used in 485 modules
EE_ASYNC_CONFIG_LSB EQU     0x05    ; ** Not used in 485 modules
...
EE_STREAM_ANALOG    EQU     0x10    ; ** Not used in 485 modules
EE_ANALOG_CFG_BASE  EQU     0x11    ; ** Not used in 485 modules
EE_STREAM_DIG       EQU     0x19    ; ** Not used in 485 modules
EE_STREAM_COUNT     EQU     0x1A    ; ** Not used in 485 modules
...
ADC_CS          EQU     0x00
EEPROM_CS       EQU     0x01
RS485_ENABLE    EQU     0x02
RTC_CS          EQU     0x03
DAC_CS          EQU     0x05

; PORT E
DELAY_JUMPER    EQU     0x00
...
MOD_ADDRESS     EQU     RamBank0+0x0D
DEST_ADDRESS    EQU     RamBank0+0x0E
TRANSMIT_WAIT   EQU     RamBank0+0x0F
SRC_ADDRESS     EQU     RamBank0+0x10
FAST_RESPONSE   EQU     RamBank0+0x11

INPUT_REGISTER  EQU     RamBank0+0x12
SCRATCH         EQU     RamBank0+0x13
CONTROL_ANALOG  EQU     RamBank0+0x14
RCV_ERROR_CNT   EQU     RamBank0+0x15
...
UNIPOLAR_FLAG   EQU     RamBank0+0x1D
...
; ------------- BANK 0 ------------------------------------------------------
        ...
        bcf     PORTA, RS485_ENABLE ; RS485 in Receive mode
...
Start_Read_Eeprom
        movlw   EE_PORT1_DIR    ; Direction PORT1 (PORTB)
        call    Read_EEPROM
        movwf   OUR_TRISB
        bsf     STATUS, RP0     ; select BANK 1
        ...
        movlw   EE_PORT2_DIR    ; Direction PORT2 (PORTD)
        call    Read_EEPROM
        movwf   OUR_TRISD
        bsf     STATUS, RP0     ; select BANK 1
        ...
        movlw   EE_PORT1_DEF    ; Default power up output values for PORT1 (PORTB)
        call    Read_EEPROM
        xorwf   EXPANDER, W     ; EXPANDER Opto-22 inversion
        movwf   PORTB

        movlw   EE_PORT2_DEF    ; Default power up output values for PORT2 (PORTD)
        call    Read_EEPROM
        xorwf   EXPANDER, W     ; EXPANDER Opto-22 inversion
        movwf   PORTD
...
;
        ...
        clrf    DEST_ADDRESS        ; Clear address variables for conversion
        clrf    SRC_ADDRESS

        movfw   RCV_BUFFER+0    ; Get the DEST address
        call    Hex_To_Nibble   ; Convert the HEX nibble
        movwf   DEST_ADDRESS
        swapf   DEST_ADDRESS, F     ; swap to high order nibble location
        movfw   RCV_BUFFER+1
        call    Hex_To_Nibble   ; Convert the HEX nibble
        iorwf   DEST_ADDRESS, F

        movfw   MOD_ADDRESS     ; Is the packet Destination our address?
        xorwf   DEST_ADDRESS, W
        btfsc   STATUS, Z
        goto    Main_Loop_Got_Address

        movfw   DEST_ADDRESS        ; Is the packet Destination 0xFF?
        xorlw   0xFF
        btfss   STATUS, Z
...
Main_Loop_Got_Address
        ...
        call    Hex_To_Nibble   ; Convert the HEX nibble
        movwf   SRC_ADDRESS
        swapf   SRC_ADDRESS, F      ; Swap to the high order nibble
        movfw   RCV_BUFFER+3
        call    Hex_To_Nibble   ; Convert the HEX nibble
        iorwf   SRC_ADDRESS, F
...
Main_Loop_Check_V
        ...
        goto    Main_Loop_Error_Response
                                    ; Command/Packet OK
        call    Transmit_Address    ; Transmits address (and any delays)
        movlw   'V'
        call    Transmit_Char
...
Main_Loop_Check_Q
        ...
        goto    Main_Loop_Error_Response
                                    ; Command/Packet OK
        clrf    UNIPOLAR_FLAG       ; Bipolar sample
...
Main_Loop_Check_U
        ...
        goto    Main_Loop_Error_Response
                                    ; Command/Packet OK
        movlw   0x01
        movwf   UNIPOLAR_FLAG       ; Unipolar sample
        movfw   RCV_BUFFER+5        ; Get the ANALOG CONTROL nibble
        call    Hex_To_Nibble
        movwf   CONTROL_ANALOG
        call    Process_Analog
...
Process_Analog
        call    Acquire_Sample_1296 ; Acquire the sample from ADC
        call    Transmit_Address    ; Transmits address (and any delays)
        movf    UNIPOLAR_FLAG, F    ; Is the UNIPOLAR flag set?
        btfss   STATUS, Z
...
Main_Loop_Rcv_End
        bcf     PORTA, RS485_ENABLE ; Disable RS-485 driver
...
;----------------------------------------------------------------------------
; Waits 1-2 character times and transmits SRC DEST addresses
;
Transmit_Address
...
Transmit_No_Wait
        bsf     PORTA, RS485_ENABLE ; Enable the RS-485
        movfw   SRC_ADDRESS
        andlw   0xF0
        movwf   HEX_TEMP
        swapf   HEX_TEMP, W         ; swap into low order nibble
        call    Nibble_To_Hex
        call    Transmit_Char
        movfw   SRC_ADDRESS
        andlw   0x0F
...
;
Read_EEPROM_Config
        movlw   EE_MOD_ADDR         ; MODULE address
        call    Read_EEPROM
        movwf   MOD_ADDRESS
        movlw   EE_DEST_ADDR        ; Async DESTINATION address
        call    Read_EEPROM
        movwf   DEST_ADDRESS
...
Read_EEPROM_Config_E
        ...
        movlw   0x31                    ; SPI Master mode, CKP=1, Fosc/16 = 921,600 Hz
...
;
Check_DIP_Switch
        btfsc   PORTE, DELAY_JUMPER
        goto    Check_DIP_Delay1
        movlw   TRUE
        movwf   FAST_RESPONSE
        goto    Check_DIP_SW0
...
Check_DIP_SW0
        ...
        andlw   0x06            ; Mask everything but bits 1,2
        xorwf   DIP_SWITCH, W
        btfsc   STATUS, Z
...
Check_DIP_SW1
        movfw   DIP_SWITCH
        xorlw   0x02            ; Val=2, Pos1=ON Pos2=OFF, Baud=19200
        btfss   STATUS, Z
        goto    Check_DIP_SW2
...
Check_DIP_SW2
        movfw   DIP_SWITCH
        xorlw   0x04            ; Val=4, Pos1=OFF Pos2=ON, Baud=57600
        btfss   STATUS, Z
        goto    Check_DIP_SW3
...
Check_DIP_SW3                   ; Val=6, Pos1=ON Pos2=ON, BAUD=115200
...
; Enable RS485 drive, send character, then disable RS485 drive
Transmit_Char
        movwf   TXREG
        bsf     STATUS, RP0     ; select BANK 1
Transmit_Pending
...
Acquire_Sample_1296
        ...
        movlw   0x0F                    ; to mask off bits to ensure good conversion
        andwf   CONTROL_ANALOG, F
        movlw   0x10                    ; START=1
        iorwf   CONTROL_ANALOG, W
        call    Spi_Send                ; Send off 1 part of control byte
        movf    UNIPOLAR_FLAG, F        ; Is the UNIPOLAR flag set?
        btfss   STATUS, Z
...
Acquire_Sample2
        call    Spi_Send                ; Send off part 2 of control byte,
                                        ; get first nibble back ftom LTC1296
        andlw   0x0F                    ; Mask off MSB nibble
        movwf   ADC_NIBBLE1
        call    Spi_Send                ; Get nibbles 2 and 3 from LTC1296
        movwf   SCRATCH
        andlw   0x0F                    ; Mask off nibble 3
        movwf   ADC_NIBBLE3
        movlw   0xF0                    ; Mask off nibble 2 and swap into lower position
        andwf   SCRATCH, F
        swapf   SCRATCH, W
        movwf   ADC_NIBBLE2
