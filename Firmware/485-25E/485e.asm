...
;
; Version 1.6
;   Changed algorithm to use table driven 485 drive enable timeout rather
;   than calculate timeout.
;
;--------------------------------------------------------------------------
;
; Author
...
TMR0_Timeout    EQU     ScratchPadRam+4
Bit_Count       EQU     ScratchPadRam+5
...
RS232_TMT
                ...
                MOVLW   0xFF            ; Reset minimum bit count
                MOVWF   Bit_Count_Min
                MOVLW   0x09            ; Opcode count to here
                MOVWF   Bit_Count
...
RS232_TMT_1     MOVFW   Bit_Count       ; Check new bit count minimum
                SUBWF   BitCount_Min, W ; subtract current from current min.
                BTFSS   STATUS, C
                GOTO    RS232_TMT_OFF   ; no carry set (Timeout < W)
                MOVFW   Bit_Count       ; carry set (Timeout > W)
                MOVWF   Bit_Count_Min   ; Save bit count
...
RS232_TMT_OFF
                ...
                SUBWF   TMR0_Timeout, W
                BTFSS   STATUS, C
                GOTO    RS232_TMT_TIMEOUT
                GOTO    RS232_TMT_OFF

RS232_TMT_2     MOVFW   Bit_Count       ; Check new bit count minimum
                SUBWF   BitCount_Min, W ; subtract current from current min.
                BTFSS   STATUS, C
