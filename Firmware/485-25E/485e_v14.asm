...
RS232_TMT_ON
                ...
                GOTO    RS232_TMT_1
                MOVLW   0xAF            ; Check if TMR0 count > 175
                SUBWF   TMR0, W
                BTFSC   STATUS, C
                CALL    RS232_SLOW
                GOTO    RS232_TMT_ON
...
;--------------------------------------------------------------------------
;
; Changes the Timer0 prescaler to 1:128 and resets Timeout parameters to
; relfect a worse case of 600 BPS
;
RS232_SLOW
                MOVLW   0xC6            ; Option register prescale /128
                OPTION
                CLRF    TMR0
                INCF    TMR0, F
                MOVLW   0x82            ; Starting timeout 130 count, 600 BPS
                MOVWF   Timeout_X10
                MOVLW   0x0D            ; bit time timeout 13
                MOVWF   Timeout
                RETLW   0
