// USBM3000_GUIDlg.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CUSBM3000_GUIDlg dialog functions
//
CUSBM3000_GUIDlg::CUSBM3000_GUIDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CUSBM3000_GUIDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CUSBM3000_GUIDlg)
	m_Command_Edit = _T("V");
	m_Response_Edit = _T("");
	m_sId_Static = _T("");
	m_dwModule_Address = 0;
	m_sStream_Static = _T("");
	//}}AFX_DATA_INIT
...
void CUSBM3000_GUIDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CUSBM3000_GUIDlg)
	...
	DDX_Text(pDX, IDC_ADDRESS_EDIT, m_dwModule_Address);
	DDX_Text(pDX, IDC_STREAM_STATIC, m_sStream_Static);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CUSBM3000_GUIDlg, CDialog)
	//{{AFX_MSG_MAP(CUSBM3000_GUIDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_DUPL_BUTTON, OnDuplButton)
	...
	//}}AFX_MSG_MAP
//	ON_LBN_SELCHANGE(IDC_DEVICE_LIST, OnLbnSelchangeDeviceList)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CUSBM3000_GUIDlg message handlers

BOOL CUSBM3000_GUIDlg::OnInitDialog()
{
    DWORD   i;
    CString sTemp;
...
//	SetIcon(m_hIcon, FALSE);		// Set small icon
    ...
    for (i=1; i<= m_Usbm300_Dev.m_dwUSBM300DeviceCount; i++) {
        sTemp.Format("USBM400 device #%i", i);
        m_Device_List.AddString(sTemp);
    }

    if (m_Usbm300_Dev.m_dwUSBM300DeviceCount >= 1) {
...
void CUSBM3000_GUIDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CUSBM3000_GUIDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}


void CUSBM3000_GUIDlg::OnCancel() 
{
    //
...
void CUSBM3000_GUIDlg::OnRefreshButton() 
{   
    CWaitCursor cWait;
    DWORD   i;
    CString sTemp;
    ...
    m_Usbm300_Dev.Enum_USBM300_Devices();

    // Add the USBM300 device list
    for (i=1; i<= m_Usbm300_Dev.m_dwUSBM300DeviceCount; i++) {
        sTemp.Format("USBM400 device #%i", i);
        m_Device_List.AddString(sTemp);
    }
...
void CUSBM3000_GUIDlg::OnTimer(UINT nIDEvent) 
{
    int     DeviceIndex;
    ...
    if (nIDEvent == 1) {
        ...
        if (DeviceIndex != LB_ERR) {
            if (!m_Usbm300_Dev.SendCommand(DeviceIndex, "V", sResp)) {
                //
...
void CUSBM3000_GUIDlg::OnIdentButton() 
{
    CButton *pButton;
    int     DeviceIndex;
    ...
    DeviceIndex = m_Device_List.GetCurSel();
    if (DeviceIndex == LB_ERR) {
        MessageBox("A USBM400 device instance must be selected", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
        return;
    }
...
void CUSBM3000_GUIDlg::OnSendButton() 
{
    CWaitCursor cWait;
    int     DeviceIndex;
    char    sResp[MAX_CMD_RESP];
    ...
    // Send the command
    //
    if (!m_Usbm300_Dev.SendCommand(DeviceIndex, sCmd, sResp)) {
...
void CUSBM3000_GUIDlg::OnDuplButton() 
{
    CWaitCursor cWait;
    int         DeviceIndex;
    DWORD       dwCount = 0;
    ...
    // Check for duplicate module addresses
    if (m_Usbm300_Dev.DuplicateAddressPresent()) {
        MessageBox("Duplicate USBM300 Module addresses present", "INFO USBM300 GUI", MB_OK | MB_ICONINFORMATION);
    } else {
        MessageBox("No duplicate USBM300 module addresses found", "INFO USBM300 GUI", MB_OK | MB_ICONINFORMATION);
    }
...
void CUSBM3000_GUIDlg::OnStreamButton() 
{
    CWaitCursor cWait;
    int         DeviceIndex;
    ...
    if (szFileData == NULL) {
        MessageBox("Cannot create file write buffer", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    if (hDataFile == INVALID_HANDLE_VALUE) {
        MessageBox("Cannot create temp data file", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    if (m_Usbm300_Dev.GetStreamingStatus(DeviceIndex, &dwStreamBlocksAvailable, &dwThreadState)) {

        if (dwThreadState & USB_THREAD_RUNNING) {
            // Thread already running. Error
            MessageBox("Stream mode thread already running", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
            if (hDataFile != INVALID_HANDLE_VALUE) {
            ...
            m_Usbm300_Dev.ControlStreaming(DeviceIndex, TRUE, 500);
        }
    } else {
        MessageBox("Error getting stream thread status", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
    }
    ...
    while (dwEndTime > dwCurrentTime) {
        ...
        bStatus = m_Usbm300_Dev.GetStreamingData(DeviceIndex,   // Current selected index
        ...
        if (!bStatus) {
            MessageBox("Error getting stream data", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
            break;
        } 
    ...
    if (m_Usbm300_Dev.GetStreamingStatus(DeviceIndex, &dwStreamBlocksAvailable, &dwThreadState)) {
        ...
        if (dwThreadState & USB_THREAD_RUNNING) {
            ...
            m_Usbm300_Dev.ControlStreaming(DeviceIndex, FALSE, 500);
        } else {
            // No thread running. Error
            MessageBox("Stream mode thread not running", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
        }
    } else {
        MessageBox("Error getting stream thread status", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
    }
    ...
    MessageBox(sTemp, "USBM300 Stream test results", MB_OK | MB_ICONINFORMATION);
...
void CUSBM3000_GUIDlg::OnGetAddress() 
{
    CWaitCursor cWait;
    int         DeviceIndex;
    LONG        lModuleAddress;
    ...
    //
    // Clear the Module address string, redraw the window, and give the user some time to see it blank.
    //
    UpdateData(FALSE);
    pEdit->SetWindowText("");
    pEdit->RedrawWindow();
    ...
    lModuleAddress = m_Usbm300_Dev.GetModuleAddress(DeviceIndex);
    if (lModuleAddress == -1) {
        MessageBox("Cannot get module address", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
        return;
    }

    // Save off new module address
    m_dwModule_Address = (DWORD)lModuleAddress;
...
void CUSBM3000_GUIDlg::OnSetAddress() 
{
    CWaitCursor cWait;
    int         DeviceIndex;
    char        sResp[MAX_CMD_RESP];
    ...
    if ((m_dwModule_Address < 1) || (m_dwModule_Address > 255)) {
        MessageBox("Module Address out of range\r\nModule Address = 1...255", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    //
    pEdit->SetWindowText("Setting address...");
    pEdit->RedrawWindow();
    Sleep(100);


    // Build command to write EEPROM address 0x00 with module address
    sprintf(sCmd, "W00%02X", m_dwModule_Address);
    if (!m_Usbm300_Dev.SendCommand(DeviceIndex, sCmd, sResp)) {
        MessageBox("Error setting Module Address", "ERROR USBM300 GUI", MB_OK | MB_ICONERROR);
    }
...
void CUSBM3000_GUIDlg::OnSpeedButton() 
{
#if 1
    int         DeviceIndex;
    char        sResp[MAX_CMD_RESP];
    ...
    while (!m_bStopLoop) {
        ...
        // Send a data packet to the unit
        //
        if (!m_Usbm300_Dev.SendCommand(DeviceIndex, "U08", sResp)) {
            //
            // Stop the loop on error
            //
            break;
...
//void CUSBM3000_GUIDlg::OnLbnSelchangeDeviceList()
//{
