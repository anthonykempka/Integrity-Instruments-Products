...
char Copyright[] = "Copyright (c) 1996-1998 Integrity Instruments";
...
//
// Here is a sample Main routine that uses the above RS232 communications function
//
void main (void)
{
    HANDLE  ComHandle;              // Handle to the communications port.
    char    Data_Buffer[100] = {0}; // Zero the Data buffer
    DWORD   BytesRead;
    ...
    //
    ComHandle = Open_Comport (2, 115200);

    if (ComHandle == 0) {
        printf ("Comport Open FAIL!\n");
        return;
    } else {
        printf ("Comport Open SUCCESS\n");
    }

    //
    // Copy data into buffer
    //
    strcpy (Data_Buffer, "V\r");
    Write_Comport (ComHandle, strlen (Data_Buffer), Data_Buffer);

    //
    // Do whatever processing or Wait for a response
    //
    Sleep (50);

    //
    // Read any receive data into our local Data Buffer
    //
    Read_Comport (ComHandle, &BytesRead, sizeof(Data_Buffer), Data_Buffer);
    printf ("Bytes Read=%i\n", BytesRead);
