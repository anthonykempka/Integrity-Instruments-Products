...
//
// Description:
//  Tests the 485-POW board. Assumes a 485-25A data converter connected
//  to ONLY ONE 485-POW board
// 
//
// 
//
...
int main (int argc, char *argv[])
{
    ...
    printf ("ID485_DIG TEST PROGRAM v1.2\n");
    ...
    i = Read_EEPROM (0x01, 0x01, &Val);
    while (i) {
        printf ("**** CAN'T COMMUNICATE TO MODULE1 ****\n");
        printf ("**** Error = %i\n", i);
        if (kbhit ()) {
            ch = getch();
            if (ch == 27) 
    ...
    while (!done) {
        ...
        if (Querry_Inputs (0x01, &Val)) {
            printf ("Can't communicate!\n");
        } else {
            Set_Outputs (0x01, &OutVal);
            Querry_Outputs (0x01, &Val2);
            printf ("INPUTS=%04X OUTPUTS=%04X\r", Val, Val2);
        }
