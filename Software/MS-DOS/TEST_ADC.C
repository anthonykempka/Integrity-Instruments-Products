...
//
// Description:
//  Tests the 485-ADC board. Assumes a 485-25A data converter connected
//  to ONLY ONE 485-ADC board
// 
//
// Last Modified:       
...
#define ERR_INVALID_CHANNEL     0x0002
#define ERR_INVALID_PARAMETER   0x0003
#define ERR_NO_RESPONSE         0x0004
#define ERR_INVALID_RESPONSE    0x0005
...
//
//
SHORT Get_Analog (USHORT Module_Address, USHORT Channel, USHORT *Value)
{
    ...
    *Value = Module_Value [Module_Address] [Channel];
...
//
SHORT Get_SW_Version (USHORT Module_Address, USHORT *Value)
{
    SHORT DataLen;
    UCHAR DataBuffer[128];
...
void PrintJustified (USHORT Val1, USHORT Val2, USHORT Val3, USHORT Val4) 
{
    CHAR strn[128];
    CHAR str[128];
    SHORT i;

    sprintf (str, "%5.3f", Cnts_to_Volts_Uni (Val1));
    strcpy (strn, str);
    strncat (strn, "        ", 9-strlen(str));
    sprintf (str, "%5.3f", Cnts_to_Volts_Bi (Val1));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));

    sprintf (str, "%5.3f", Cnts_to_Volts_Uni (Val2));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));
    sprintf (str, "%5.3f", Cnts_to_Volts_Bi (Val2));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));

    sprintf (str, "%5.3f", Cnts_to_Volts_Uni (Val3));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));
    sprintf (str, "%5.3f", Cnts_to_Volts_Bi (Val3));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));

    sprintf (str, "%5.3f", Cnts_to_Volts_Uni (Val4));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));
    sprintf (str, "%5.3f", Cnts_to_Volts_Bi (Val4));
    strcat (strn, str);
    strncat (strn, "        ", 9-strlen(str));
...
int main (int argc, char *argv[])
{
    ...
    USHORT Val1, Val2, Val3, Val4;

    USHORT OutVal = 0x0001;

    USHORT  Mod_Config = 0x00;
    USHORT  temp;
   
    printf ("ID485_ADC TEST PROGRAM v1.2\n");
    ...
    if (!Configure_IO(0x01, &Mod_Config)) {
        printf ("Module IO configured and RESET!\n");
    } else {
        printf ("Can't configure module IO!\n");
    }
    ...
    while (!done) {
        if (kbhit ()) {
            ...
            if (ch == '1') {
                ...
                if (!Configure_IO(0x01, &Mod_Config)) {
                    printf ("Module IO configured to %04x and RESET!\n", Mod_Config);
                } else {
                    printf ("Can't configure module IO!\n");
                }
            }
        ...
        if (!Querry_Analog(0x01, &Val1, &Val2, &Val3, &Val4)) {
            PrintJustified (Val1, Val2, Val3, Val4);
        } else {
            printf ("Error reading Analog!\n");
        }
    }                                   // while (!done)
