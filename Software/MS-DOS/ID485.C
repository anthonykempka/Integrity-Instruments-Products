...
//---------------------------------------------------------------------------
//
// Analog data that is sent ONLY on an Asyn Update time base.
//
//
...
//---------------------------------------------------------------------------
//
//  Resets the module after the Response!
//
...
int main (int argc, char *argv[])
{
    ...
    //
    // Timer0 may be used in lieu of RTC. Substitute InitializeRtcIsr()
    // with a call to InitializeTimer0Isr()
    //
    ...
    while (!done) {
        ...
        if (kbhit()) {                  // Check for a keyboard event
            ...
            if (i==0) {                 // check for extended keys.
                ...
                if (i == '2') {
                    Val = 0;
                    printf ("Querry_SW_VERSION=%i\n", Querry_SW_Version(0x01, &Val));
                    printf ("SW=v%i.%i\n", (Val & 0xFF00)>>8, Val & 0xFF);
                }

                if (i == '3') {
                    Val = 0;
                    printf ("Querry_Inputs=%i\n", Querry_Inputs(0x01, &Val));
                    printf ("Inputs=%04X\n", Val);
                }
                ...
                if (i == '5') {
                    Val = 20000;
                    printf ("Set Update_Time=%i\n", Set_Update_Time(0x01, &Val));
                    printf ("Inputs=%04X\n", Val);
                }
                ...
                printf ("Async=%li Sync=%li\n", AsyncPacketsRcvd, SyncPacketsRcvd);
    ...
    //
    // Restore Timer0 if used.  Call RestoreTimer0Isr()
    //
